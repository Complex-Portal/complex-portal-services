...
mvn clean install -P import-qsproteome-structures,${PROFILE} -Djami.user.context.id=${USER_ID} -Djob.name=complexQsProteomeImport -Doutput.directory=$OUTPUT_DIRECTORY -Dseparator=$SEPARATOR -Dheader=$HEADER -DskipTests
