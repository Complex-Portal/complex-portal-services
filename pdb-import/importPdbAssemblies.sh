...
mvn clean install -P import-pdb-assemblies,${PROFILE} -Djami.user.context.id=${USER_ID} -Djob.name=pdbAssembliesImport -Dinput.file.name=$INPUT_FILE -Doutput.directory=$OUTPUT_DIRECTORY -Dseparator=$SEPARATOR -Dheader=$HEADER -DskipTests
