...
public abstract class AbstractBatchWriter<T> implements ItemWriter<T>, ItemStream {
    ...
    protected DbSynchronizerStatisticsReporter synchronizerListener;

    protected abstract FileConfiguration getFileConfiguration();
    protected abstract ComplexService getComplexService();
    ...
    public void open(ExecutionContext executionContext) throws ItemStreamException {
        ...
        getComplexService().getIntactDao().getSynchronizerContext().initialiseDbSynchronizerListener(synchronizerListener);
    }
    ...
    protected void initialiseReportWriters() throws IOException {
        File reportDirectory = new File(getFileConfiguration().getReportDirectory());
        if (!reportDirectory.exists()) {
            reportDirectory.mkdirs();
        }
        if (!reportDirectory.isDirectory()) {
            throw new IOException("The reports directory has to be a directory: " + getFileConfiguration().getReportDirectory());
        }
    }
}
