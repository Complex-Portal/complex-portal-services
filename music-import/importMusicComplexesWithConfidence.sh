...
mvn clean -U install -P import-music-complexes,${PROFILE} -Djami.user.context.id=${USER_ID} -Djob.name=musicComplexesImport -Dinput.file.name=$INPUT_FILE -Doutput.directory=$OUTPUT_DIRECTORY -Dseparator=$SEPARATOR -Dheader=$HEADER -Dmusic.cell.line=EFO:0002869 -Dmusic.input.file.fields=ids,proteins,confidence -DskipTests
